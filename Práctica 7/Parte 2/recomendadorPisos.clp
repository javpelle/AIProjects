...
(defrule INTERVIEW::entrevista
	...
	(bind ?parej (read))
	(printout t "Alguno de los ocupantes de la casa puede ser considerado una persona mayor o tiene problema de movilidad? (si no): ")
	(bind ?mayor(read))
	(printout t "Introduce el numero de coches que tienes: ")
	(bind ?coch (read))
	(printout t "Cual es su presupuesto maximo?: ")
	(bind ?dineros(read))
	(assert (cliente (pareja ?parej) (huespedes ?hij) (anciano ?mayor) (coches ?coch) (presupuesto ?dineros)))
)
